...
class SequenceSimulator():
    ...
    def simulate_shifts(self, events_file):
        ...
        self.category_position["100000"] = int(len(substitution_rates)/2)

        self.branchwise_rates = dict()
        self.branchwise_rates["100000"] = list()
        self.branchwise_rates["100000"].append((0, "S", substitution_rates[self.category_position["100000"]]))

        # Second, we compute the time to the next event:

        elapsed_time = 0.0
        self.active_genomes = set()
        self.active_genomes.add("100000")
