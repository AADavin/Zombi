...
def test_reproducibility():
    ...
    # Run Zombi with original function
    os.environ['USE_NEW_FUNCTION'] = '0'
    os.makedirs(output_folder_1, exist_ok=True)
    run_zombi('T', f'{test_parameters_dir}/SpeciesTreeParameters.tsv', output_folder_1)
    run_zombi('G', f'{test_parameters_dir}/GenomeParameters.tsv', output_folder_1)

    # Run Zombi with new function
    os.environ['USE_NEW_FUNCTION'] = '1'
    os.makedirs(output_folder_2, exist_ok=True)
    run_zombi('T', f'{test_parameters_dir}/SpeciesTreeParameters.tsv', output_folder_2)
    run_zombi('G', f'{test_parameters_dir}/GenomeParameters.tsv', output_folder_2)

    # Compare outputs
    if compare_folders(output_folder_1, output_folder_2):
        print('The outputs are identical between the two functions! ✅')
    else:
        print('The outputs are different between the two functions! ❌')
